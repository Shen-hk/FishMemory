...
class BlockEditorRecyclerView @JvmOverloads constructor(
    ...
    private fun notifyContentChanged() {
        ...
        editorCallback?.onContentChanged()
    }
    ...
    /** 设置统一编辑器事件回调，推荐新接入方优先使用。 */
    fun setEditorCallback(callback: EditorCallback?) {
        editorCallback = callback
        operations.setEditorCallback(callback)
    }

    /** 注入视频管理器：播放器与上传器。 */
    fun setVideoManagers(
        playerManager: VideoPlayerManager?,
        uploadManager: EditorVideoUploader?
    ) {
    ...
    /** 按 SDK 标准协议导出 JSON。 */
    fun exportStandardJson(): String {
        return RichTextDocumentCodec.exportStandardJson(getDocument())
    }

    /** 从 SDK 标准协议 JSON 恢复编辑器内容；解析失败返回 false。 */
    fun loadStandardJson(json: String): Boolean {
        val document = RichTextDocumentCodec.parseEditableDocument(json) ?: return false
